...
function loadProducts(href) {
   ...
   $('.category').addClass("disactive_category")
   $(".category").css("pointer-events", "none");
   $('.cards').html("<i class='fa fa-circle-o-notch loader' aria-hidden='true'></i>");

   $(".category").removeClass("active")
   $("a[href='"+href+"']").addClass("active")

   var category = href.split("/");
   var name = $(".category[href='"+href+"'] span").text();
   ...
   var ajax = $.ajax({
      ...
      success: function(response){
         ...
         $('.cards').html(response);
         $('.category').removeClass("disactive_category")
         $(".category").css("pointer-events", "auto");
         status = 1;
		 
		 if(click){
			 var target = $(".menu").children("h1");
			 var window_h = $(window).height()
			 if(window_h >= 768) plus_px = ($(".top_navbar").height() + 16) + 35;
			 else plus_px = 40;
			 $('html, body').animate({scrollTop: $(target).offset().top - plus_px}, 500);
		 }
   ...
   setTimeout(function () {
      if(status == 0){
         $('.cards').html("Time out. Please try again later");
         $('.category').removeClass("disactive_category")
         $(".category").css("pointer-events", "auto");
      }
   }, 15000);
...
$(window).on('load', function() {
   loadProducts(window.location.pathname);//When pg is loaded
